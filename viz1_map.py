...
FIG_BG = "#f9f7f0"
# Blue ocean background that doesn't overpower hotspot points.
OCEAN = "#9ecae1"
# Green land/coast polygons (subtle, behind the points).
# Dark green landmass.
LAND_FILL = "#1b4d2e"
# Blackish boundary color for stronger contrast at coast/land edges.
LAND_EDGE = "#1f1f1f"
...
if gpd is not None:
    try:
        local_land = ROOT / "NL_Land_Bound_4792812732831105689.geojson"
        if local_land.exists():
            land = gpd.read_file(local_land)
        else:
            raise FileNotFoundError(str(local_land))

        # Ensure EPSG:4326 (lon/lat) and clip to the plotting extent for speed.
        ...
        land.plot(
            ...
            edgecolor=LAND_EDGE,
            linewidth=0.28,
            # Make land polygon opaque enough to hide grid lines behind it.
            alpha=0.65,
            zorder=1,
            aspect=None,
        )
...
# Thick black outer border (editorial frame) around the map.
...
# Reference-like dark purple/red -> warm tones -> light beige gradient.
...
regions = [
    # Offsets push label boxes slightly farther offshore (longer arrows),
    # while avoiding text boxes covering the land polygons.
    # Northern Peninsula label: move up and farther offshore.
    # Northern Peninsula label: slightly lower, kept offshore.
    ("Northern\nPeninsula", "Northern Peninsula-St. Anthony", (-55, 8)),
    # South coast: lift label away from bottom axis tick labels.
    ("South coast", "South Coast-Channel-Port aux Basques", (-18, -15)),
    # Labrador label: move it noticeably higher and farther offshore.
...
# Bold black axis markings (ticks + labels) for clarity.
...
# Grid should appear in the ocean area only.
# Put grid at the very back so the (more opaque) land polygons cover it.
